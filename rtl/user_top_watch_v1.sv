...
module user_top_watch_v1 #(
  ...
  // -----------------------------------------------------------------------------
  // Core functionality: Basic watch timekeeping.
  // 1st Version. This version has no edit mode yet; it only counts time normally.
  // -----------------------------------------------------------------------------

  // Internal counter values use their natural widths.
  ...
  restartable_rate_generator #(
      ...
      .clk (clk),
      .run (1'b1),
      .tick(seconds_tick)  //if seconds_tick = 1 at rising edge of clock
                           //then seconds increase by 1
  );
  ...
  //No edit mode - no manual increment/decrement
  ...
  // Cascade timekeeping:
  // minutes tick when seconds wrap from 59 to 0,
  // hours tick when minutes and seconds both wrap.
  assign minutes_tick = seconds_tick && (seconds == 6'd59);
  assign hours_tick = seconds_tick && (minutes == 6'd59) && (seconds == 6'd59);
